...
class MyTrainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        # logger.info(f"GPU:{torch.cuda.current_device()} inputs:{inputs}")
        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, inputs)
        # logger.info(f"GPU:{torch.cuda.current_device()} loss:{loss}")
...
class SaveModelCallback(transformers.TrainerCallback):
    def save_model(self, args, state, kwargs):
        ...
        model_path = os.path.join(checkpoint_folder, "pt_model")
    ...
    def on_train_end(self, args, state, control, **kwargs):
        model_path = os.path.join(args.output_dir, "pt_model")
        kwargs["model"].save_pretrained(model_path)
        kwargs["tokenizer"].save_pretrained(model_path)
